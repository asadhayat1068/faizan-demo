  ...
  const handleOrderCreation = async () => {
    ...
    try {
      ...
      if (response && response.status === "success") {
        ...
        // const _response = await redeemProducts(
        //   response.orderId,
        //   currency.address,
        //   walletId
        // );
        // console.log("Redeem response:", _response);
              ...
              <div className="mt-4">
                ...
                <ul className="list-disc pl-5">
                  {shippingOptions.map((option, index) => (
                    <li key={index}>
                      <strong>{option.serviceType}</strong> -{" "}
                      {option.deliveryDate} - ${option.cost}
                    </li>
                  ))}
                </ul>
              </div>

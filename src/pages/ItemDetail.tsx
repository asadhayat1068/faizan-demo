...
const ItemDetail: React.FC = () => {
  ...
  const customAttributes = product.custom_attributes.reduce((acc: Record<string, any>, attr) => {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="md:w-1/2 md:pl-8 mt-4 md:mt-0">
          <h1 className="text-3xl font-bold mb-1 text-gray-800">{product.name}</h1>
          <div className='text-gray-500 font-sans flex flex-row space-x-4 mb-1'>
            <div>By Cryptovia</div>
            <img src={Seperator} />
            <div><Link key='opensea' to={`https://opensea.io/collection/cryptrovia`}>Visit OpenSea store</Link></div>
            <img src={Seperator} />
            <div><span className='font-bold'>Sku:</span> {product.sku}</div>
          </div>
          <div className="mt-2 flex items-center">
            <img src={eth} alt="eth" className="w-5 h-5" />
            <span className='text-green-700'>{product.price}</span>
            <span className='pl-2'>{'/'}</span>
            <span className="text-red-700 pl-2">${customAttributes["usd"]}</span>
          </div>
          ...
          {customAttributes["description"] && (
            <div className="mb-2 font-sans">
              ...
              <span className="text-gray-600">{stripHtmlTags(customAttributes["description"])}</span>
            </div>
          )}
        </div>
      </div>
